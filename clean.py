...
from urllib.parse import urlsplit, parse_qsl, parse_qs, urlunsplit, urlencode
...
def match_provider(provider, url, rules):
    ...
    for exception_pattern in rules['exceptions']:
        try:
            ...
            msg = (
                f'something is wrong with regex {exception_pattern!r} '
                f'for provider {provider!r} in exceptions'
            )
        if match_exception:
            break
